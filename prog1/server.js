...
mongoose.connect('mongodb://10.92.130.58:27017/blogger', { useNewUrlParser: true });
...
app.get('/blogs/edit/:_id', function(req, res) {
	Blog.getBlogById(req.params._id, async function(err, blog) {
		if(err){
			throw err;
		}
...
//check('category').isString.withMessage('Category field is only required name.')],
async function(req, res){
	...
	{
		//Pull the author, search by the id that is in the form
		var newAuthor = await User.findById(req.body.author);
		var author = { 
			'name': 'harry potter',
